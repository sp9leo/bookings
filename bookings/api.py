...
_SL_WEEKDAYS = ["ponedeljek", "torek", "sreda", "četrtek", "petek", "sobota", "nedelja"]
_SL_MONTHS = ["januar", "februar", "marec", "april", "maj", "junij", "julij", "avgust", "september", "oktober", "november", "december"]


def _format_slovenian_date(dt):
    return "{} {} {} {}".format(
        _SL_WEEKDAYS[dt.weekday()], dt.day, _SL_MONTHS[dt.month - 1], dt.year
    )
...
def _send_reservation_confirmation(result, customer_email):
    ...
    try:
        ...
        date_str = _format_slovenian_date(from_time)
  ...
  <tr>
    <td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:520px;background-color:#ffffff;border:1px solid #e5e7eb;border-radius:16px;overflow:hidden;">
        <tr>
          <td style="background-color:#4f46e5;padding:24px 28px;color:#ffffff;font-size:20px;font-weight:bold;text-align:center;">Potrditev rezervacije</td>
        </tr>
        <tr>
          <td style="padding:28px;">
            <p style="margin:0 0 20px;font-size:15px;color:#374151;">Pozdravljeni, {frappe.utils.cstr(doc.customer_name)}!</p>
            <p style="margin:0 0 20px;font-size:15px;color:#374151;">Vaša rezervacija je potrjena. Spodaj so podrobnosti:</p>
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f9fafb;border-radius:12px;padding:16px 20px;font-size:14px;">
              <tr><td style="padding:4px 0;color:#6b7280;">Termin z</td><td style="padding:4px 0;font-weight:600;text-align:right;">{frappe.utils.cstr(item_name)}</td></tr>
              <tr><td style="padding:4px 0;color:#6b7280;">Datum</td><td style="padding:4px 0;font-weight:600;text-align:right;">{date_str}</td></tr>
              <tr><td style="padding:4px 0;color:#6b7280;">Ura</td><td style="padding:4px 0;font-weight:600;text-align:right;">{from_time.strftime("%H:%M")} – {to_time.strftime("%H:%M")}</td></tr>
              <tr><td style="padding:4px 0;color:#6b7280;">Referenčna številka</td><td style="padding:4px 0;font-weight:700;text-align:right;">{frappe.utils.cstr(doc.booking_ref)}</td></tr>
            </table>
            <p style="margin:20px 0 8px;font-size:13px;color:#6b7280;">Referenčno številko shranite, da si boste rezervacijo lahko pozneje ogledali ali jo preklicali:</p>
            <a href="{manage_url}" style="display:inline-block;background-color:#4f46e5;color:#ffffff;text-decoration:none;font-weight:600;font-size:14px;padding:12px 24px;border-radius:10px;">Poglej mojo rezervacijo</a>
          </td>
        </tr>
      </table>
    </td>
...
"""
        frappe.sendmail(
            ...
            subject=f"Potrditev rezervacije — {item_name}",
            message=message,
            reference_doctype="Reservation",
            reference_name=doc.name,
            header="Potrditev rezervacije",
        )
    except Exception:
...
@frappe.whitelist()
def update_booking_time(booking_ref, new_start_time, new_end_time, scope="this"):
    """Move a room booking (or its recurring series) to a new start/end time."""
    ...
    for name in targets:
        ...
        room = doc.reservation_item
        date = str(doc.booking_date).split(" ")[0]

        target_slot_name = _available_slot_name(room, date, start_hm)

        if target_slot_name and target_slot_name != doc.available_slot:
            _increment_available_slot(
                target_slot_name,
                f"This room is fully booked at {start_hm} on {date}",
            )
            _release_available_slot(doc.available_slot)
            doc.available_slot = target_slot_name
        elif not target_slot_name:
            _release_available_slot(doc.available_slot)
            doc.available_slot = None

        if doc.schedule_slot:
            try:
                legacy = frappe.get_doc("Schedule Slot", doc.schedule_slot)
                legacy.status = "Free"
        ...
        doc.from_time = _combine_datetime(doc.booking_date, new_start_time)
        doc.to_time = _combine_datetime(doc.booking_date, new_end_time)
        doc.save(ignore_permissions=True)
...
@frappe.whitelist()
def update_booking_details(booking_ref, notes=None, customer_name=None, scope="this"):
    """Update notes and/or booked-by name on a room booking (optionally its series)."""
...
@frappe.whitelist()
def update_recurrence(booking_ref, frequency=None, interval=None, until_date=None, scope="future"):
    """Change the pattern/end of a recurring series, or unlink it.
                ...
                "bookers": [
                    {
                        ...
                        "notes": r.get("notes"),
                        "recurring_group_id": r.get("recurring_group_id"),
                    }
...
def book_room_slot(room, date, start_time, end_time, notes=None,
    ...
    booking = _insert_room_booking(
        room, date, start_hm, end_hm, notes, customer_name, customer_email,
        recurring_group_id=recurring_group_id,
        recurring_frequency=recurring_frequency,
        recurring_interval=recurring_interval,
        recurring_until_date=recurring_until_date,
    )
...
def book_room_recurring(room, dates, start_time, end_time, notes=None,
                          customer_name=None, customer_email=None,
                          frequency=None, interval=None, until_date=None):
    """Create room bookings across multiple dates for the same time,
    linked together as one recurring series."""
    import json
    ...
    for slot_date in dates:
        try:
            res = book_room_slot(
                ...
                recurring_until_date=until_date,
            )
        except Exception:
            continue
        created.append({"date": slot_date, "booking_ref": res["booking_ref"]})

    return {"success": True, "created": created, "recurring_group_id": group_id}
